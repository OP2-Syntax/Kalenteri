...
export function useEventOccurrencesWithTimelinePosition(parameters) {
  ...
  return React.useMemo(() => computeOccurrencesWithTimelinePosition(occurrences, maxSpan), [occurrences, maxSpan]);
}
export function computeOccurrencesWithTimelinePosition(occurrences, maxSpan) {
  const sortedOccurrences = sortEventOccurrences(occurrences);
  const {
    firstIndexLookup,
    lastIndexLookup,
    maxIndex
  } = buildOccurrenceIndexLookups(sortedOccurrences, maxSpan);
 ...
 */
export function computeOccurrencesMaxIndex(occurrences) {
  const sortedOccurrences = sortEventOccurrences(occurrences);
 ...
 */
export function computeOccurrencesFirstIndexLookup(occurrences) {
  const sortedOccurrences = sortEventOccurrences(occurrences);
...
function buildOccurrenceIndexLookups(occurrences, maxSpan) {
  ...
  for (let occurrenceIndex = 0; occurrenceIndex < occurrences.length; occurrenceIndex += 1) {
    ...
    if (activeIndexes) {
      if (index > 1) {
        ...
        nextConflictingIndexLookup[previousOccurrenceKey] = Math.min(nextConflictingIndexLookup[previousOccurrenceKey] ?? Infinity, index);
      }
      const activeIndexCount = activeIndexes.sum(index);
      if (activeIndexCount < activeOccurrences.length) {
        nextConflictingIndexLookup[occurrence.key] = activeIndexes.findByOrder(activeIndexCount + 1);
      }
  ...
  for (const occurrence of occurrences) {
    ...
    lastIndexLookup[occurrence.key] = Math.min(nextConflictingIndex - 1, maxIndex, firstIndex + maxSpan - 1);
  }
  return {
    firstIndexLookup,
    lastIndexLookup,
...
function popHeap(heap) {
  ...
  while (index * 2 + 1 < heap.length) {
    ...
    if (childIndex + 1 < heap.length && getHeapValue(heap[childIndex + 1]) < getHeapValue(heap[childIndex])) {
      childIndex += 1;
    }
