...
var _resizeUtils = require("../../internals/utils/resize-utils");
...
function useTimeDropTarget(parameters) {
  ...
  const getDateAtPointer = (0, _useStableCallback.useStableCallback)(input => {
    ...
    const roundedOffsetMs = Math.round(offsetMs / _constants.EVENT_DRAG_PRECISION_MS) * _constants.EVENT_DRAG_PRECISION_MS;
  ...
  const getEventDropData = (0, _useStableCallback.useStableCallback)(({
    ...
    if (data.source === 'CalendarGridTimeEventResizeHandler') {
      if (data.side === 'start') {
        ...
        const {
          start: newStartDate
        } = (0, _resizeUtils.clampResizedEventEdge)({
          adapter,
          side: 'start',
          start: data.start,
          end: data.end,
          cursorDate,
          precisionMinute: _constants.EVENT_DRAG_PRECISION_MINUTE
        });
