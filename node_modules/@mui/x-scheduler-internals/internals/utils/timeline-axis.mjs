...
export function isFullDayAxisWindow(axis) {
 ...
 */
export function isStartMinuteOutsideAxisWindow(axis, minutesInDay) {
 ...
 */
export function isEndMinuteOutsideAxisWindow(axis, minutesInDay) {
 ...
 */
export function getTimelineAxisDayMs(axis) {
 ...
 */
export function getWallClockMsInDay(adapter, date) {
  return adapter.getHours(date) * HOUR_MS + adapter.getMinutes(date) * MINUTE_MS + adapter.getSeconds(date) * 1000 + adapter.getMilliseconds(date);
}
function getTotalDays(adapter, axis) {
 ...
 */
export function getTimelineAxisDurationMs(adapter, axis) {
 ...
 */
export function timelineAxisOffsetToDate(adapter, axis, offsetMs) {
 ...
 */
export function dateToTimelineAxisOffsetMs(adapter, axis, date, msInDay = getWallClockMsInDay(adapter, date)) {
  const dayMs = getTimelineAxisDayMs(axis);
  const dayIndex = adapter.differenceInDays(adapter.startOfDay(date), adapter.startOfDay(axis.start));
  const clampedMsInDay = Math.min(Math.max(msInDay, axis.dayStartMinute * MINUTE_MS), axis.dayEndMinute * MINUTE_MS);
