...
exports.getDisplayedHourRange = getDisplayedHourRange;
 ...
 */
function getDisplayedHourRange(startTime, endTime, source) {
  ...
  if (!isValid) {
    if (process.env.NODE_ENV !== 'production') {
      (0, _warning.warnOnce)([`MUI X Scheduler: \`${source}\` received an invalid hour range (startTime: ${resolvedStartTime}, endTime: ${resolvedEndTime}).`, '`startTime` and `endTime` must be whole hours between 0 and 24 with `startTime` lower than `endTime`.', 'Falling back to the full day (0–24).']);
    }
    return {
