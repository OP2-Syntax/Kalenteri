...
import { getOccurrenceKey, getRecurringOccurrenceKey } from "../event-utils.mjs";
...
export class SchedulerStore extends Store {
  ...
  constructor(parameters, adapter, instanceName, mapper, recurringEventsPlugin = null) {
    ...
    this.disposables.defer(this.registerStoreEffect(state => state.adapter.getTime(state.visibleDate), this.stopEditing));
  ...
  updateStateFromParameters = (parameters, adapter) => {
    ...
    if (!parameters.dataSource && (parameters.events !== this.parameters.events || parameters.eventModelStructure !== this.parameters.eventModelStructure || adapter !== this.state.adapter || newSchedulerState.displayTimezone !== this.state.displayTimezone)) {
      Object.assign(newSchedulerState, buildEventsState({
        events: parameters.events,
        eventModelStructure: parameters.eventModelStructure,
        adapter,
  ...
  pushError = (error, options) => {
    ...
    if (options?.transient) {
      const existing = this.state.errors.find(entry => this.transientErrorKeys.has(entry.key) && entry.error.message === wrapped.message);
    ...
    if (options?.transient) {
      ...
      this.timeoutManager.startTimeout(`transient-error-${key}`, TRANSIENT_ERROR_DURATION_MS, () => this.dismissError(key));
    }
    return key;
  };
  ...
  selectRecurringEventScope = scope => {
    ...
    if (pendingRecurringEventOperation.kind === 'update') {
      ...
      } = pendingRecurringEventOperation.changes;
      // Only repoint when the resized occurrence is the armed one, else a sibling drag hijacks the surface.
      const {
        editingOccurrence
      } = this.state;
      const resizedOccurrenceKey = getRecurringOccurrenceKey(eventId, occurrenceStartInDataTimezone, adapter);
   ...
   */
  setOccurrencePlaceholder = (newPlaceholder, event) => {
    const {
      adapter,
   ...
   */
  requestEditingStart(occurrence, event, trigger, anchor) {
    ...
    let eventDetails;
    if (isCreation) {
      eventDetails = createChangeEventDetails('creation', event ?? this.occurrencePlaceholderEvent, trigger, {
        occurrence: occurrence,
        anchor: resolvedAnchor
      });
    } else {
      // The dialog renders view-only content for read-only occurrences — surface that as its own reason.
      const reason = schedulerEventSelectors.isReadOnly(this.state, occurrence.id) ? 'view' : 'edit';
   ...
   */
  startEditing = (occurrence, mode = 'edit', event, trigger, anchor) => {
   ...
   */
  setEditingMode = (mode, event, trigger, anchor) => {
    const {
      editingOccurrence
    } = this.state;
    ...
    if (mode === 'edit' && !this.requestEditingStart(editingOccurrence.occurrence, event, trigger, anchor)) {
      this.stopEditing();
      return;
    }
    this.set('editingOccurrence', _extends({}, editingOccurrence, {
      mode
    }));
  };
   ...
   */
  setEditingOccurrenceTimes = (start, end) => {
    const {
      editingOccurrence,
      adapter
    } = this.state;
    ...
    this.set('editingOccurrence', _extends({}, editingOccurrence, {
      occurrence: _extends({}, occurrence, {
        displayTimezone: _extends({}, occurrence.displayTimezone, {
   ...
   */
  repointEditingOccurrence = (eventId, start, end, isRecurring, dataTimezone) => {
    const {
      editingOccurrence,
      adapter
    } = this.state;
    ...
    this.set('editingOccurrence', _extends({}, editingOccurrence, {
      occurrence: _extends({}, occurrence, {
        ...
        getRecurringOccurrenceKey(eventId, adapter.setTimezone(start, dataTimezone), adapter) : getOccurrenceKey(eventId),
        displayTimezone: _extends({}, occurrence.displayTimezone, {
          start: processDate(start, adapter),
          end: processDate(end, adapter),
