...
exports.isBuiltInEventProperty = isBuiltInEventProperty;
 ...
 */
function isBuiltInEventProperty(key) {
...
function getCustomEventProperties(model) {
  ...
  for (const key in model) {
    ...
    if (Object.prototype.hasOwnProperty.call(model, key) && !EVENT_PROPERTIES_LOOKUP.hasOwnProperty(key)) {
      setOwnProperty(customProperties, key, model[key]);
    }
  }
...
function buildEventsState(options) {
  const {
    adapter,
    displayTimezone,
    previousState,
  ...
  const events = options.events ?? _empty.EMPTY_ARRAY;
  const recurringEventsPlugin = options.recurringEventsPlugin ?? previousState?.recurringEventsPlugin ?? null;
  const eventIdList = [];
  const eventModelLookup = new Map();
  const processedEventLookup = new Map();
  const previousContext = previousState ? processedEventLookupContext.get(previousState.processedEventLookup) : undefined;
  const canReusePrevious = previousState !== undefined && previousContext !== undefined && previousContext[0] === adapter && previousContext[1] === displayTimezone && previousContext[2] === eventModelStructure && previousContext[4] === recurringEventsPlugin;
  const previousProcessedEventByModel = canReusePrevious ? previousContext[3] : null;
  const processedEventByModel = new WeakMap();
  let hasSameEventIds = canReusePrevious;
  let eventModelMismatchCount = 0;
  let processedEventMismatchCount = 0;
  for (const event of events) {
    const processedEvent = previousProcessedEventByModel?.get(event) ?? getProcessedEventFromModel(event, adapter, eventModelStructure, displayTimezone, recurringEventsPlugin);
    ...
    if (alreadySeen) {
      ...
      if (hasSameEventIds && canReusePrevious && previousState.eventIdList[eventIdList.length] !== id) {
  ...
  hasSameEventIds && (hasSameEventIds = canReusePrevious && eventIdList.length === previousState.eventIdList.length);
  ...
  const nextProcessedEventLookup = hasSameProcessedEvents ? previousState.processedEventLookup : processedEventLookup;
  processedEventLookupContext.set(nextProcessedEventLookup, [adapter, displayTimezone, eventModelStructure, processedEventByModel, recurringEventsPlugin]);
  return {
