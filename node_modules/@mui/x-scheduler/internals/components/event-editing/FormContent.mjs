...
import { getCustomEventProperties, getEventResourceIds, getResourceSelectionMode, isBuiltInEventProperty } from '@mui/x-scheduler-internals/internals';
import { useEventEditingStyledContext } from "./EventEditingStyledContext.mjs";
import { useEventEditingOptionalRenderers } from "./EventEditingOptionalRenderersContext.mjs";
import { computeRange, findInvalidRangeField, getInvalidValueErrorMessage, getRangeErrorMessage, validateRange, hasProp, BUILT_IN_FORM_KEYS } from "../event-dialog/utils.mjs";
import EventDialogHeader from "../event-dialog/EventDialogHeader.mjs";
import TitleSection from "../event-dialog/TitleSection.mjs";
import { GeneralTab } from "../event-dialog/GeneralTab.mjs";
import { EventDialogFormProvider, useEventDialogFormContext } from "../event-dialog/form/EventDialogFormContext.mjs";
import { eventDialogFormSelectors } from "../event-dialog/form/EventDialogFormStore.mjs";
import { usePushPlaceholder } from "../event-dialog/usePushPlaceholder.mjs";
...
export function FormContent(props) {
  ...
  const canHaveMultipleResources = useStore(store, schedulerEventSelectors.canHaveMultipleResources);
  ...
  const resourceSelectionMode = useRefWithInit(() => getResourceSelectionMode(occurrence.resource, canHaveMultipleResources, isCreating)).current;
  ...
  const initialValues = useRefWithInit(() => {
    ...
    const customProperties = Object.fromEntries(Object.entries(model ? getCustomEventProperties(model) : {}).filter(([, value]) => value !== undefined));
    if (process.env.NODE_ENV !== 'production') {
      for (const key of Object.keys(customProperties)) {
        if (BUILT_IN_FORM_KEYS.has(key)) {
          warnOnce([`MUI X Scheduler: The event model contains a custom property "${key}" that collides with a built-in form key.`, 'The form seeds that key from the event dates and resource, so the custom property cannot be read or written through the form.', 'Rename the property in the event model to avoid the collision.']);
        }
      }
    }
    return _extends({}, customProperties, {
  ...
  return /*#__PURE__*/_jsx(EventDialogFormProvider, {
    initialValues: initialValues,
    occurrence: occurrence,
    resourceSelectionMode: resourceSelectionMode,
    onValuesChange: pushPlaceholder,
    children: /*#__PURE__*/_jsx(FormContentInner, _extends({}, props))
  });
}
function FormContentInner(props) {
  ...
  const warnUnvalidatedField = (field, problem) => {
    if (process.env.NODE_ENV !== 'production' && !formStore.hasValidator(field)) {
      warnOnce([`MUI X Scheduler: ${problem} but no field of the event dialog validates the "${field}" field.`, 'Saving is still blocked, but the end user may have no visible field to fix it.', 'Render the missing section in the General tab, or register a validator for the field.']);
    }
  };
  ...
  const runSubmitChecks = (values, start, end, current) => {
    ...
    if (isMissingRequiredResource && eventDialogFormSelectors.error(formStore.state, 'resourceIds') === undefined) {
    ...
    const invalidRangeField = findInvalidRangeField(current.adapter, values, current.displayTimezone);
    if (invalidRangeField) {
      ...
      if (eventDialogFormSelectors.error(formStore.state, invalidRangeField) === undefined) {
        formStore.setError(invalidRangeField, getInvalidValueErrorMessage(invalidRangeField, localeText));
      }
    }
    ...
    const rangeError = invalidRangeField ? null : validateRange(current.adapter, start, end, values.allDay);
    if (rangeError) {
  ...
  const handleSubmit = async event => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (shouldEventRequireResource && !formStore.hasValidator('resourceIds')) {
        warnOnce(['MUI X Scheduler: `shouldEventRequireResource` is enabled but no field of the event dialog validates the resource.', 'Saving without a resource is still blocked, but the end user has no visible field to fix it.', 'Render the resource section in the General tab, or register a validator for the "resourceIds" field.']);
      }
    }
    ...
    try {
      ...
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          warnOnce(['MUI X Scheduler: A form field validator threw or rejected during the submit.', 'The submit was aborted and no error was stored on the form.', 'Handle failures inside the validator and return the error message instead.']);
        }
        return;
      }
      if (!isSessionAliveRef.current) {
      ...
      const current = {
        ...
        recurrencePresets: schedulerRecurringEventSelectors.presets(store.state, occurrence.displayTimezone.start)
      };
      const values = formStore.state.values;
      const {
        start,
        end
      } = computeRange(current.adapter, values, current.displayTimezone);
      ...
      const metaChanges = _extends({}, editedCustomValues, {
      ...
      if (rawPlaceholder?.type === 'creation') {
        ...
        store.updateEvent(_extends({}, metaChanges, {
          id: occurrence.id,
          start,
          end,
          rrule: rruleToSubmit
  ...
  return /*#__PURE__*/_jsx(DialogContent, {
    ...
    children: /*#__PURE__*/_jsxs(EventDialogForm, {
      ...
      children: [/*#__PURE__*/_jsx(EventDialogHeader, {
        ...
        children: /*#__PURE__*/_jsx(TitleSection, {})
      ...
      }), /*#__PURE__*/_jsx(FormActionButtons, {
...
function FormActionButtons(props) {
  ...
  return /*#__PURE__*/_jsxs(FormActions, {
