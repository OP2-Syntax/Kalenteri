...
function normalizeValidatorResult(result) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (list.some(message => typeof message === 'boolean')) {
      warnOnce(['MUI X Scheduler: A form field validator returned a boolean.', 'Booleans are ignored: return the error message(s) when the value is invalid, or `null` when it is valid.']);
    }
  }
 ...
 */
export class EventDialogFormStore extends Store {
  validators = new Map();
  ...
  constructor(initialValues, parameters) {
    ...
    this.initialValues = _extends({}, initialValues);
  ...
  setValues = changes => {
    ...
    const values = hasValueChange ? _extends({}, this.state.values, resolvedChanges) : this.state.values;
  ...
  setError = (key, error) => {
    ...
    this.set('errors', _extends({}, this.state.errors, {
      [key]: messages
    }));
  };
  ...
  seedDefault = (key, value) => {
    ...
    this.set('values', _extends({}, this.state.values, {
      [key]: value
    }));
  };
    ...
    for (let restarts = 0;; restarts += 1) {
      ...
      if (isSettled || restarts >= maxRestarts) {
        if (process.env.NODE_ENV !== 'production' && !isSettled) {
          warnOnce(['MUI X Scheduler: The form values or validators kept changing while the validation was running (for example a validator calling setValue).', 'The submit stays blocked; the stored errors describe the last completed pass.', 'Avoid writing values or (un)registering validators from a validator.']);
        }
