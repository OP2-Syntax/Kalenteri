...
import { useEventEditingContext, useEventEditingStyledContext } from "../event-editing/index.mjs";
import { useDisarmOnEscape } from "../armed-occurrence/index.mjs";
...
export function EventToolbar(props) {
  const {
    occurrence
  } = props;
  ...
  const areRecurringEventsAvailable = useStore(store, schedulerOtherSelectors.areRecurringEventsAvailable);
  ...
  const handleEdit = event => {
    ...
    store.setEditingMode('edit', event.nativeEvent, event.currentTarget, stableAnchorRef.current ?? anchor ?? undefined);
  };
