...
export function useAnchoredPosition(parameters) {
  const {
    anchor,
    popupRef,
    side = 'left',
  ...
  React.useEffect(() => {
    ...
    const resizeObserver = typeof ResizeObserver !== 'undefined' && popup ? new ResizeObserver(followKeepingDrag) : null;
    if (popup) {
      resizeObserver?.observe(popup);
    }
    const mutationObserver = typeof MutationObserver !== 'undefined' && anchor ? new MutationObserver(followKeepingDrag) : null;
    if (anchor) {
